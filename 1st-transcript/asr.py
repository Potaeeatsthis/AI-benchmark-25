...
if __name__ == "__main__":
    ...
    if args.resume_from_checkpoint:
        model = ASRModule.load_from_checkpoint(
        ...
        model = ASRModule(
            ...
            learning_rate=learning_rate,
            warmup_steps=warmup_steps,
            total_steps=total_steps,  # Pass correct computed value :contentReference[oaicite:30]{index=30}
        )
    ...
    ckpt_path_for_trainer = args.resume_from_checkpoint if args.resume_from_checkpoint else None
    ...
    sigterm_ckpt = os.path.join(args.output_dir, "sigterm.ckpt")

    def _save_on_sigterm(signum, frame):
        print(f"\nReceived SIGTERM (signal {signum}), saving checkpoint to {sigterm_ckpt} …")
        try:
            trainer.save_checkpoint(sigterm_ckpt)
            print(f"Checkpoint saved at {sigterm_ckpt}")
        except Exception as e:
            print("Failed to save checkpoint on SIGTERM:", e)
        finally:
            sys.exit(0)
    ...
    try:
        ...
        interrupted_ckpt = os.path.join(args.output_dir, "interrupted.ckpt")
        trainer.save_checkpoint(interrupted_ckpt)
        print(f"Checkpoint saved at:\n    {interrupted_ckpt}")
    
    final_ckpt = os.path.join(args.output_dir, "final.ckpt")
